...
def main(args):
    ...
    for file in os.listdir(args.data_dir):
        ...
        denoised = denoise_and_bens_prepro(img)
        image_tensor = test_tx(denoised)
        image_tensor = image_tensor.unsqueeze_(0)
        inp = Variable(image_tensor)
        inp = inp.to(device, dtype=torch.float)
        predict = model(inp)
        reg=  Image.new('RGB', img.size)
        out_img,reg = _nms(args, img, predict, nms_score=0.3, iou_threshold=0.1, reg=reg)
        print('saving image to ', args.output_dir + file )
        Image.fromarray(out_img).save(args.output_dir + file)
        Image.fromarray(np.asarray(reg)).save(args.output_dir+'_reg_' + file)
    
    
def _nms(args, img, predict, nms_score, iou_threshold, reg):
...
#         print(str(top-bottom))
        ...
        font = ImageFont.truetype('fonts/NomNaTong-Regular.ttf', int(min( (right-left),(bottom-top) )*3/5) )
